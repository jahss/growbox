...
const ELEMENT_IDS = [
  ...
  'useRateInputs', 'useRateSummary', 'useRateResult', 'useRateNitrogen', 'useRateCopy',
...
test('blend target is entered in elemental ppm and a solve shows delivered ppm', () => {
  ...
  assert.match(elements.blendInputs.innerHTML, /N ppm<input id="btN" class="bi" data-k="N"/);
...
test('page ids are unique and never clash with ids the N-form fields generate', () => {
  const html = require('node:fs').readFileSync(require('node:path').join(__dirname, '..', 'index.html'), 'utf8');
  const ids = [...html.matchAll(/\sid="([^"]+)"/g)].map(match => match[1]);
  assert.deepEqual(ids.filter((id, index) => ids.indexOf(id) !== index), []);
  ['ga', 'bc', 'bt'].flatMap(group => [group + 'N', group + 'NToggle', group + 'NForms'])
    .forEach(id => assert.ok(!ids.includes(id), id + ' is generated by an N-form field and must not be a page id'));
});
