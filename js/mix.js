(function (root, factory) {
  ...
  function createComponent(options) {
    ...
    function render(containerId, lines) {
      ...
      box.innerHTML = '<h2>Mix it</h2><p class="muted">Turn this recipe into what to weigh out.</p>' +
        '<div class="seg mix-seg"><button type="button" data-mode="reservoir"' + (mix.mode === 'reservoir' ? ' class="active"' : '') + '>Reservoir</button><button type="button" data-mode="stock"' + (mix.mode === 'stock' ? ' class="active"' : '') + '>Injector stock</button></div>' +
      ...
      box.querySelectorAll('.mixIn').forEach(input => {
        const update = () => {
          ...
          save();
          box.querySelector('.mix-out').innerHTML = outputHtml(lines, mix);
        };
  ...
  return Object.freeze({GAL_L, RATIOS, reservoirAmounts, stockPlan, createComponent});
});
