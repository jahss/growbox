...
const PRODUCTS=window.FERTILIZER_PRODUCTS||[],SYSTEMS=window.FERTILIZER_SYSTEMS||[],LEVELS=Array.from({length:((300-50)/10)+1},(_,i)=>50+i*10),P_LEVELS=Array.from({length:((150-10)/10)+1},(_,i)=>10+i*10),K_LEVELS=Array.from({length:((300-50)/10)+1},(_,i)=>50+i*10);
...
const ANALYSIS_COMPONENT=ANALYSIS.createComponent({document,chemistry:CHEM,levels:LEVELS,pLevels:P_LEVELS,kLevels:K_LEVELS,format:fmt});
...
function render(){tabs();COMPARE_COMPONENT.render();USE_RATE_COMPONENT.render();manual();BLEND_COMPONENT.render()}render();
})();
